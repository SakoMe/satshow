...
describe('PATCH /api/v1/gateway-modem/:id', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/gateway-modem/:id', async () => {
    const response = await request(app).patch('/api/v1/gateway-modem/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).patch('/api/v1/gateway-modem/:id').expect(401);
  });

  it('returns a 500 if the gateway does not exist', async () => {
    await request(app)
      .patch('/api/v1/gateway-modem/0')
      .set('Cookie', global.setAuthCookie())
      .expect(500);
  });

  it('returns a 500 if the gateway is updated with invalid or incomplete data', async () => {
    ...
    const modemResponse = await request(app)
      .post('/api/v1/gateway-modem')
      .set('Cookie', global.setAuthCookie())
      .send({
        latitude: 40,
        longitude: 40,
    ...
    const modem = modemResponse.body;

    const response = await request(app)
      .patch(`/api/v1/gateway-modem/${modem.id}`)
      .set('Cookie', cookie)
      .send({ latitude: 'lat' });
  ...
  it('returns a 200 if the gateway is updated', async () => {
    const cookie = await getAuthCookie();
    const credentials = { email: 'valid@mail.com', password: 'hello123' };
    const user = await createUser(credentials.email, credentials.password);
    const planResponse = await request(app)
    ...
    expect(response.body.id).toEqual(modem.id);
  });
});
