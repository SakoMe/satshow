...
describe('PATCH /api/v1/segment-gateway/:id', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/segment-gateway/:id', async () => {
    const response = await request(app).patch('/api/v1/segment-gateway/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).patch('/api/v1/segment-gateway/:id').expect(401);
  });

  it('returns a 500 if the gateway does not exist', async () => {
    await request(app)
      .patch('/api/v1/segment-gateway/0')
  ...
  it('returns a 500 if the gateway is updated with invalid or incomplete data', async () => {
    ...
    const response = await request(app)
      .patch(`/api/v1/segment-gateway/${gateway.id}`)
      .set('Cookie', cookie)
      .send({
        gateway_name: 0,
        latitude: 40,
  ...
  it('returns a 200 if the gateway is updated', async () => {
    ...
    const response = await request(app)
      ...
      .set('Cookie', cookie)
      .send({
        gateway_name: 'different gateway',
        latitude: 40,
        longitude: 40,
