...
describe('DELETE /api/v1/connectivity-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/connectivity-collection', async () => {
    const response = await request(app).del('/api/v1/connectivity-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).del('/api/v1/connectivity-collection').expect(401);
  });

  it('returns a 422 if invalid or incomplete connectivities data is sent for deletion', async () => {
    await request(app)
      .del('/api/v1/connectivity-collection')
      .set('Cookie', global.setAuthCookie())
      .send({ beam_type: 'fail' })
      .expect(422);
  });

  it('returns a 200 if connectivities are deleted', async () => {
    ...
    expect(response.body).toEqual(connectivities);
  });
});
