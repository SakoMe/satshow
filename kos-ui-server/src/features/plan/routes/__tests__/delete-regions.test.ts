...
describe('DELETE /api/v1/region-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/region-collection', async () => {
    const response = await request(app).del('/api/v1/region-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).del('/api/v1/region-collection').expect(401);
  });

  it('returns a 500 if invalid or incomplete regions data is sent for deletion', async () => {
    await request(app)
      .del('/api/v1/region-collection')
      .set('Cookie', global.setAuthCookie())
      .send({ region_name: 'fail' })
      .expect(500);
  });

  it('returns a 200 if regions are deleted', async () => {
    ...
    expect(response.body).toEqual(regions);
  });
});
