...
describe('PATCH /api/v1/plan/:id', () => {
  it('has a route handler listening to /api/v1/plan/:id', async () => {
    const response = await request(app).patch('/api/v1/plan/:id');
  ...
  it('returns a 401 if the user is not authenticated', async () => {
    ...
    await request(app).patch(`/api/v1/plan/${plan.id}`).expect(401);
  });

  it('returns a 200 if a plan is updated', async () => {
    const cookie = await getAuthCookie();
    const credentials = { email: 'valid@mail.com', password: 'hello123' };
    const user = await createUser(credentials.email, credentials.password);
    const planResponse = await request(app)
    ...
    const response = await request(app)
      .patch(`/api/v1/plan/${plan.id}`)
      .set('Cookie', cookie)
      .send({
        plan_name: 'different name',
        number_segments: 2,
        time_period_start: new Date(2023, 2, 8),
        time_period_end: new Date(2023, 2, 9),
      });
    ...
    expect(response.body.plan_name).not.toEqual(plan.plan_name);
  });
});
