...
describe('PATCH /api/v1/service-segment/:id', () => {
  it('has a route handler listening to /api/v1/service-segment/:id', async () => {
    const response = await request(app).patch('/api/v1/service-segment/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).patch('/api/v1/service-segment/:id').expect(401);
  });

  it('returns a 500 if the segment does not exist', async () => {
    await request(app)
      .patch('/api/v1/service-segment/0')
      .set('Cookie', global.setAuthCookie())
      .expect(500);
  });

  it('returns a 500 if the segment is updated with invalid data', async () => {
    ...
    await request(app)
      .patch(`/api/v1/service-segment/${segment.id}`)
      .set('Cookie', cookie)
      .send({
        service_segment_name: 0,
        service_segment_type: SERVICE_SEGMENT_TYPE.DTH,
        service_segment_data_type: SERVICE_SEGMENT_DATA_TYPE.Bandwidth,
        service_segment_priority: 1,
        service_segment_gateway_handover: false,
        defrag_userlink_allowed: false,
        service_segment_gain_mode: GAIN_MODE_TYPE.ALC,
        plan_id: plan.id,
  ...
  it('returns a 200 if the segment is updated', async () => {
    const cookie = await getAuthCookie();
    const credentials = { email: 'valid@mail.com', password: 'hello123' };
    const user = await createUser(credentials.email, credentials.password);
    const planResponse = await request(app)
    ...
    const response = await request(app)
      ...
      .set('Cookie', cookie)
      .send({
        service_segment_name: 'different segment',
        service_segment_type: SERVICE_SEGMENT_TYPE.DTH,
        service_segment_data_type: SERVICE_SEGMENT_DATA_TYPE.Bandwidth,
        service_segment_priority: 1,
        service_segment_gateway_handover: false,
    ...
    expect(response.body.id).toEqual(segment.id);
    expect(response.body).not.toEqual(segment);
  });
});
