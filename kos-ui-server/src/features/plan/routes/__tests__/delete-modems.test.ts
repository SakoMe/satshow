...
describe('DELETE /api/v1/gateway-modem-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/gateway-modem-collection', async () => {
    const response = await request(app).del('/api/v1/gateway-modem-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).del('/api/v1/gateway-modem-collection').expect(401);
  });

  it('returns a 422 if invalid or incomplete modems data is sent for deletion', async () => {
    await request(app)
      .del('/api/v1/gateway-modem-collection')
      .set('Cookie', global.setAuthCookie())
      .send({ latitude: 'lat' })
      .expect(422);
  });

  it('returns a 200 if modems are deleted', async () => {
    ...
    expect(response.body).toEqual(modems);
  });
});
