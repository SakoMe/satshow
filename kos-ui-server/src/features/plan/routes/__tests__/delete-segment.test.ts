...
describe('DELETE /api/v1/service-segment/:id', () => {
  it('has a route handler listening to /api/v1/service-segment/:id', async () => {
    const response = await request(app).del('/api/v1/service-segment/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).del('/api/v1/service-segment/:id').expect(401);
  });

  it('returns a 500 if the segment does not exist', async () => {
    await request(app)
      .del('/api/v1/service-segment/0')
      .set('Cookie', global.setAuthCookie())
      .expect(500);
  });

  it('returns a 200 if a segment is created under a valid user', async () => {
    ...
    const response = await request(app)
      .del(`/api/v1/service-segment/${segment.id}`)
      .set('Cookie', cookie);
