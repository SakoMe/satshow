...
describe('POST /api/v1/gateway-modem-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/gateway-modem-collection', async () => {
    const response = await request(app).post('/api/v1/gateway-modem-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/gateway-modem-collection').expect(401);
  });

  it('returns a 422 if invalid or incomplete gateways data is sent', async () => {
    await request(app)
  ...
  it('returns a 201 if gateways are created under a valid user', async () => {
