...
describe('PATCH /api/v1/connectivity/:id', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/connectivity/:id', async () => {
    const response = await request(app).patch('/api/v1/connectivity/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).patch('/api/v1/connectivity/:id').expect(401);
  });

  it('returns a 500 if the connectivity does not exist', async () => {
    await request(app)
      .patch('/api/v1/connectivity/0')
      .set('Cookie', global.setAuthCookie())
      .expect(500);
  });

  it('returns a 500 if the connectivity is updated with invalid or incomplete data', async () => {
    ...
    const connectivity = connectivityResponse.body;

    const response = await request(app)
      .patch(`/api/v1/connectivity/${connectivity.id}`)
      .set('Cookie', cookie)
      .send({});
  ...
  it('returns a 200 if the connectivity is updated', async () => {
    const cookie = await getAuthCookie();
    const credentials = { email: 'valid@mail.com', password: 'hello123' };
    const user = await createUser(credentials.email, credentials.password);
    const planResponse = await request(app)
    ...
    const response = await request(app)
      ...
      .send({
        ...
        satellite_eirp: 0,
        bandwidth: 1,
        temporal_variations: [{ time: 'time', demand: 9 }],
        modcods: [{ name: 'modcod', spec_eff_ideal: 1, spec_eff: 0, esno: 0 }],
        service_segment_id: segment.id,
      });

    expect(response.status).toEqual(200);
    expect(response.body.id).toEqual(connectivity.id);
  });
});
