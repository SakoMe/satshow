...
describe('POST /api/v1/segment-gateway-collection', () => {
  it('has a route handler listening to /api/v1/segment-gateway-collection', async () => {
    const response = await request(app).post('/api/v1/segment-gateway-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/segment-gateway-collection').expect(401);
  });
