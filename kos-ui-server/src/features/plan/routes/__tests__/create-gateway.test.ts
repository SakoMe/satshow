...
describe('POST /api/v1/segment-gateway', () => {
  it('has a route handler listening to /api/v1/segment-gateway', async () => {
    const response = await request(app).post('/api/v1/segment-gateway');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/segment-gateway').expect(401);
  });

  it('returns a 422 if invalid or incomplete gateway data is sent', async () => {
    await request(app)
