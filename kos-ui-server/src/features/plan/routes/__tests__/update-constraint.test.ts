...
describe('PATCH /api/v1/constraint/:id', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/constraint/:id', async () => {
    const response = await request(app).patch('/api/v1/constraint/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).patch('/api/v1/constraint/:id').expect(401);
  });

  it('returns a 500 if the constraint does not exist', async () => {
    await request(app)
      .patch('/api/v1/constraint/0')
      .set('Cookie', global.setAuthCookie())
      .expect(500);
  });

  it('returns a 500 if the constraint is updated with invalid or incomplete data', async () => {
    ...
    const constraintResponse = await request(app)
    ...
    const constraint = constraintResponse.body;

    const response = await request(app)
      .patch(`/api/v1/constraint/${constraint.id}`)
      .set('Cookie', cookie)
      .send({
        regulatory_constraint_name: 0,
        direction: LINK_DIRECTION_TYPE.DownLink,
        polarization: POLARIZATION_TYPE.LHCP,
        min_frequency: -10,
        max_frequency: 20,
  ...
  it('returns a 200 if the constraint is updated', async () => {
    const cookie = await getAuthCookie();
    const credentials = { email: 'valid@mail.com', password: 'hello123' };
    const user = await createUser(credentials.email, credentials.password);
    const planResponse = await request(app)
    ...
    const response = await request(app)
      ...
      .set('Cookie', cookie)
      .send({
        regulatory_constraint_name: 'different constraint',
    ...
    expect(response.body.id).toEqual(constraint.id);
  });
});
