...
describe('DELETE /api/v1/method-terminal-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/method-terminal-collection', async () => {
    const response = await request(app).del('/api/v1/method-terminal-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).del('/api/v1/method-terminal-collection').expect(401);
  });

  it('returns a 422 if invalid or incomplete methods data is sent for deletion', async () => {
    await request(app)
      .del('/api/v1/method-terminal-collection')
      .set('Cookie', global.setAuthCookie())
      .send({ beam_type: 'fail' })
      .expect(422);
  });

  it('returns a 200 if methods are deleted', async () => {
