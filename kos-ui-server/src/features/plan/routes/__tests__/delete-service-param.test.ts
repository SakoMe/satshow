...
describe('DELETE /api/v1/service-param/:id', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/service-param/:id', async () => {
    const response = await request(app).del('/api/v1/service-param/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).del('/api/v1/service-param/:id').expect(401);
  });

  it('returns a 500 if the param does not exist', async () => {
    await request(app)
      .del('/api/v1/service-param/0')
      .set('Cookie', global.setAuthCookie())
      .expect(500);
  });

  it('returns a 200 if the param is deleted', async () => {
    ...
    const response = await request(app)
      .del(`/api/v1/service-param/${param.id}`)
      .set('Cookie', cookie);
