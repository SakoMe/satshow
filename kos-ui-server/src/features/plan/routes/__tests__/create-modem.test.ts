...
describe('POST /api/v1/gateway-modem', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/gateway-modem', async () => {
    const response = await request(app).post('/api/v1/gateway-modem');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/gateway-modem').expect(401);
  });

  it('returns a 422 if invalid or incomplete modem data is sent', async () => {
    await request(app)
  ...
  it('returns a 500 if no gateway exists', async () => {
    await request(app)
      .post('/api/v1/gateway-modem')
      .set('Cookie', global.setAuthCookie())
      .send({
  ...
  it('returns a 201 if a modem is created under a valid user', async () => {
