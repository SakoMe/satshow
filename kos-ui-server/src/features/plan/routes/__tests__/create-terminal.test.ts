...
describe('POST /api/v1/segment-terminal', () => {
  it('has a route handler listening to /api/v1/segment-terminal', async () => {
    const response = await request(app).post('/api/v1/segment-terminal');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/segment-terminal').expect(401);
  });

  it('returns a 422 if invalid or incomplete terminal data is sent', async () => {
    await request(app)
  ...
  it('returns a 201 if a terminal is created under a valid user', async () => {
