...
describe('POST /api/v1/constraint', () => {
  it('has a route handler listening to /api/v1/constraint', async () => {
    const response = await request(app).post('/api/v1/constraint');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/constraint').expect(401);
  });

  it('returns a 422 if incomplete constraint data is sent', async () => {
    await request(app)
  ...
  it('returns a 201 if a constraint is created under a valid user', async () => {
