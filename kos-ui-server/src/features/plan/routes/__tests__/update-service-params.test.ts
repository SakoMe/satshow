...
describe('PATCH /api/v1/service-param-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/service-param-collection', async () => {
    const response = await request(app).patch('/api/v1/service-param-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).patch('/api/v1/service-param-collection').expect(401);
  });

  it('returns a 422 if params do not exist or are updated with invalid data', async () => {
    await request(app)
      .patch('/api/v1/service-param-collection')
      .set('Cookie', global.setAuthCookie())
      .expect(422);
  });

  it('returns a 500 if params are updated with invalid data', async () => {
    ...
    const params = paramsResponse.body;
    const updatedParams = JSON.parse(JSON.stringify(params)) as typeof params;
    updatedParams[0].band = 0;
    updatedParams[1].frequency_start = -20;
    expect(params).not.toStrictEqual(updatedParams);
  ...
  it('returns a 200 if params are updated', async () => {
    const cookie = await getAuthCookie();
    const credentials = { email: 'valid@mail.com', password: 'hello123' };
    const user = await createUser(credentials.email, credentials.password);
    const planResponse = await request(app)
    ...
    const params = paramsResponse.body;
    const updatedParams = JSON.parse(JSON.stringify(params)) as typeof params;
    updatedParams[0].band = FREQUENCY_BAND_TYPE.C;
    updatedParams[1].frequency_start = 15;
    expect(params).not.toStrictEqual(updatedParams);
    ...
    expect(response.body[0].id).toEqual(params[0].id);
  });
});
