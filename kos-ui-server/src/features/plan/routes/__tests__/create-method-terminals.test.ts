...
describe('POST /api/v1/method-terminal-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/method-terminal-collection', async () => {
    const response = await request(app).post('/api/v1/method-terminal-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/method-terminal-collection').expect(401);
  });

  it('returns a 422 if invalid or incomplete methods data is sent', async () => {
    await request(app)
  ...
  it('returns a 201 if methods are created under a valid user', async () => {
