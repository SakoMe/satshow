...
describe('DELETE /api/v1/region/:id', () => {
  ...
  it('has a route handler listening to /api/v1/region/:id', async () => {
    const response = await request(app).del('/api/v1/region/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).del('/api/v1/region/:id').expect(401);
  });
  ...
  it('returns a 200 if the region is deleted', async () => {
