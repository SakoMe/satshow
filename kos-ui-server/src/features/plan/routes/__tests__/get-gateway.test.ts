...
describe('GET /api/v1/segment-gateway/:id', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/segment-gateway/:id', async () => {
    const response = await request(app).get('/api/v1/segment-gateway/:id');
  ...
  it('returns a 401 if the user is not authenticated', async () => {
    ...
    const response = await request(app).get(`/api/v1/segment-gateway/${gateway.id}`);
  ...
  it('returns a 200 if a gateway has been gotten from a valid user', async () => {
    ...
    const response = await request(app)
      .get(`/api/v1/segment-gateway/${gateway.id}`)
      .set('Cookie', cookie);
