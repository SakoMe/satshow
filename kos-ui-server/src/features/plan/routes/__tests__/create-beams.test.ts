...
describe('POST /api/v1/candidate-beam-collection', () => {
  it('has a route handler listening to /api/v1/candidate-beam-collection', async () => {
    const response = await request(app).post('/api/v1/candidate-beam-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/candidate-beam-collection').expect(401);
  });
  ...
  it('returns a 201 if beams are created under a valid user', async () => {
