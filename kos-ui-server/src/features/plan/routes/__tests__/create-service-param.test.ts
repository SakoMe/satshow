...
describe('POST /api/v1/service-param', () => {
  it('has a route handler listening to /api/v1/service-param', async () => {
    const response = await request(app).post('/api/v1/service-param');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/service-param').expect(401);
  });

  it('returns a 422 if incomplete param data is sent', async () => {
    await request(app)
  ...
  it('returns a 201 if a param is created under a valid user', async () => {
