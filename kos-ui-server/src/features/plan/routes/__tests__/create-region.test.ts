...
describe('POST /api/v1/region', () => {
  it('has a route handler listening to /api/v1/region', async () => {
    const response = await request(app).post('/api/v1/region');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/region').expect(401);
  });

  it('returns a 422 if incomplete region data is sent', async () => {
    await request(app)
      .post('/api/v1/region')
  ...
  it('returns a 500 if invalid region data is sent', async () => {
    await request(app)
      .post('/api/v1/region')
      .set('Cookie', global.setAuthCookie())
      .send({
  ...
  it('returns a 201 if a region is created under a valid user', async () => {
