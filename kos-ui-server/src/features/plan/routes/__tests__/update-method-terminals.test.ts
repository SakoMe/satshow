...
describe('PATCH /api/v1/method-terminal-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/method-terminal-collection', async () => {
    const response = await request(app).patch('/api/v1/method-terminal-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).patch('/api/v1/method-terminal-collection').expect(401);
  });

  it('returns a 422 if methods do not exist', async () => {
    await request(app)
      .patch('/api/v1/method-terminal-collection')
      .set('Cookie', global.setAuthCookie())
      .expect(422);
  });

  it('returns a 500 if methods are updated with invalid data', async () => {
    ...
    const methodsResponse = await request(app)
      .post('/api/v1/method-terminal-collection')
      .set('Cookie', cookie)
      .send([
        {
        ...
        {
          terminal_method: TERMINAL_METHOD_TYPE.Random_uniform,
          avg_num_of_termninals_per_square_km: 10,
          method_terminals: [
            {
              latitude: 40,
              longitude: 40,
    ...
    const methods = methodsResponse.body;
    const updatedMethods = JSON.parse(JSON.stringify(methods)) as typeof methods;
    updatedMethods[0].terminal_method = 0;
    updatedMethods[1].avg_num_of_termninals_per_square_km = 'km';
    expect(methods).not.toStrictEqual(updatedMethods);
  ...
  it('returns a 200 if methods are updated', async () => {
    const cookie = await getAuthCookie();
    const credentials = { email: 'valid@mail.com', password: 'hello123' };
    const user = await createUser(credentials.email, credentials.password);
    const planResponse = await request(app)
    ...
    const methods = methodsResponse.body;
    const updatedMethods = JSON.parse(JSON.stringify(methods)) as typeof methods;
    updatedMethods[0].terminal_method = TERMINAL_METHOD_TYPE.Uniform_grid;
    updatedMethods[1].avg_num_of_termninals_per_square_km = 1000;
    expect(methods).not.toStrictEqual(updatedMethods);
    ...
    expect(response.body[0].id).toEqual(methods[0].id);
  });
});
