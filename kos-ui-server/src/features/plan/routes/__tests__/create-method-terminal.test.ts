...
describe('POST /api/v1/method-terminal', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/method-terminal', async () => {
    const response = await request(app).post('/api/v1/method-terminal');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/method-terminal').expect(401);
  });

  it('returns a 422 if invalid or incomplete method data is sent', async () => {
    await request(app)
      .post('/api/v1/method-terminal')
      .set('Cookie', global.setAuthCookie())
      .send({ terminal_method: 0 })
      .expect(422);
  });
  ...
  it('returns a 201 if a method is created under a valid user', async () => {
