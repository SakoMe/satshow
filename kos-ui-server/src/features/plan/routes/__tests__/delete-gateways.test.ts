...
describe('DELETE /api/v1/segment-gateway-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/segment-gateway-collection', async () => {
    const response = await request(app).del('/api/v1/segment-gateway-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).del('/api/v1/segment-gateway-collection').expect(401);
  });

  it('returns a 422 if invalid or incomplete gateways data is sent for deletion', async () => {
    await request(app)
      .del('/api/v1/segment-gateway-collection')
      .set('Cookie', global.setAuthCookie())
      .send({ gateway_name: 0 })
      .expect(422);
  });

  it('returns a 200 if gateways are deleted', async () => {
    ...
    expect(response.body).toEqual(gateways);
  });
});
