...
describe('PATCH /api/v1/method-terminal/:id', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/method-terminal/:id', async () => {
    const response = await request(app).patch('/api/v1/method-terminal/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).patch('/api/v1/method-terminal/:id').expect(401);
  });

  it('returns a 500 if the method does not exist', async () => {
    await request(app)
      .patch('/api/v1/method-terminal/0')
      .set('Cookie', global.setAuthCookie())
      .expect(500);
  });

  it('returns a 500 if the method is updated with invalid or incomplete data', async () => {
    ...
    const response = await request(app)
      .patch(`/api/v1/method-terminal/${method.id}`)
      .set('Cookie', cookie)
      .send({
        terminal_method: 0,
        avg_num_of_termninals_per_square_km: 1,
  ...
  it('returns a 200 if the beam is updated', async () => {
    ...
    const response = await request(app)
      ...
      .set('Cookie', cookie)
      .send({
        terminal_method: TERMINAL_METHOD_TYPE.Uniform_grid,
        avg_num_of_termninals_per_square_km: 1,
        method_terminals: [
          {
            latitude: 40,
