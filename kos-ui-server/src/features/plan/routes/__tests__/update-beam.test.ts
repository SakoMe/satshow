...
describe('PATCH /api/v1/candidate-beam/:id', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/candidate-beam/:id', async () => {
    const response = await request(app).patch('/api/v1/candidate-beam/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).patch('/api/v1/candidate-beam/:id').expect(401);
  });

  it('returns a 500 if the beam does not exist', async () => {
    await request(app)
      .patch('/api/v1/candidate-beam/0')
      .set('Cookie', global.setAuthCookie())
      .expect(500);
  });

  it('returns a 500 if the beam is updated with invalid or incomplete data', async () => {
    ...
    const beamResponse = await request(app)
    ...
    const beam = beamResponse.body;

    const response = await request(app)
      .patch(`/api/v1/candidate-beam/${beam.id}`)
      .set('Cookie', cookie)
      .send({
        beam_type: 0,
        beam_diameter: 1,
  ...
  it('returns a 200 if the beam is updated', async () => {
    const cookie = await getAuthCookie();
    const credentials = { email: 'valid@mail.com', password: 'hello123' };
    const user = await createUser(credentials.email, credentials.password);
    const planResponse = await request(app)
    ...
    expect(response.body.id).toEqual(beam.id);
  });
});
