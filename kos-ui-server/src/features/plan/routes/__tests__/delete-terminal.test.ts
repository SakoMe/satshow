...
describe('DELETE /api/v1/segment-terminal/:id', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/segment-terminal/:id', async () => {
    const response = await request(app).del('/api/v1/segment-terminal/:id');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).del('/api/v1/segment-terminal/:id').expect(401);
  });

  it('returns a 500 if the terminal does not exist', async () => {
    await request(app)
      .del('/api/v1/segment-terminal/0')
      .set('Cookie', global.setAuthCookie())
      .expect(500);
  });

  it('returns a 200 if a terminal is deleted', async () => {
    ...
    const response = await request(app)
      .del(`/api/v1/segment-terminal/${terminal.id}`)
      .set('Cookie', cookie);
