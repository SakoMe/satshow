...
describe('PATCH /api/v1/region-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/region-collection', async () => {
    const response = await request(app).patch('/api/v1/region-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).patch('/api/v1/region-collection').expect(401);
  });

  it('returns a 422 if regions do not exist or are updated with invalid data', async () => {
    await request(app)
      .patch('/api/v1/region-collection')
      .set('Cookie', global.setAuthCookie())
      .expect(422);
  });

  it('returns a 200 if regions are updated', async () => {
    ...
    const regions = regionsResponse.body;
    const updatedRegions = JSON.parse(JSON.stringify(regions)) as typeof regions;
    updatedRegions[0].region_name = 'different region 1';
    updatedRegions[1].region_name = 'different region 2';
    expect(regions).not.toStrictEqual(updatedRegions);
