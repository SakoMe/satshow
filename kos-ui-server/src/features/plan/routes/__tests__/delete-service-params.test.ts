...
describe('DELETE /api/v1/service-param-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/service-param-collection', async () => {
    const response = await request(app).del('/api/v1/service-param-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).del('/api/v1/service-param-collection').expect(401);
  });

  it('returns a 500 if invalid or incomplete params data is sent', async () => {
    await request(app)
      .del('/api/v1/service-param-collection')
      .set('Cookie', global.setAuthCookie())
      .send({ band: 'fail' })
      .expect(500);
  });

  it('returns a 200 if params are deleted', async () => {
