...
describe('POST /api/v1/connectivity-collection', () => {
  jest.setTimeout(10000);

  it('has a route handler listening to /api/v1/connectivity-collection', async () => {
    const response = await request(app).post('/api/v1/connectivity-collection');

    expect(response.status).not.toEqual(404);
  });

  it('returns a 401 if the user is not authenticated', async () => {
    await request(app).post('/api/v1/connectivity-collection').expect(401);
  });

  it('returns a 422 if invalid or incomplete connectivities data is sent', async () => {
    await request(app)
  ...
  it('returns a 201 if connectivities are created under a valid user', async () => {
